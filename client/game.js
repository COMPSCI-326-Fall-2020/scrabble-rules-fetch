...
export class Game {
    ...
    /**
     * This utility function will return the full word created when placing a word at a given position, looking in the
     * given direction.
     */
    _extend(word, position, direction) {
        let extended = word;
    ...
    _score(word, position, direction) {
        const extended = this._extend(word, position, direction);
    ...
    playAt(word, position, direction) {
        ...
        for (let i = 0; i < word.length; ++i) {
            ...
            const extended = this._extend(word[i], coordinate, !direction);
            if (extended.length !== 1) {
                if (!dictionary.includes(extended)) {
                    throw `The word '${extended}' is not in the dictionary.`;
                }
